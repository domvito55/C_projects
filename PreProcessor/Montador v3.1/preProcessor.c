...
/*********************Includes do Matheus*******************/
/*arquivo cabecalho para incluir algumas funcoes micelaneas*/
#include"funcmice.h"
...
FILE *meutemp = fopen("meutemp.txt", "w+");
...
printf("%c", ' ');
		...
		if(c == '\0'){
	 	...
	 	/*simbolos de adicao, subtracao, multiplicacao e divisao foram adicionados por Matheus*/
...
/*		TransformToDecimal(&operacao, arquivo, &c, &temp);*/
		if((isdigit(c))||((c)=='-')){
			conta(&operacao, arquivo, &c, &temp);
		}
		/****************************Fim de Codigo Adicionado por Matheus******************************/

		/* a estrutura if foi adicionado por Matheus, mas seu conteudo faz parte do codigo original*/
		if(operacao == 0){
			fputc(c, temp);
printf("%c", c);
			c = fgetc(*arquivo);
